...
function drawCharts(data) {
  ...
  const byPeriod = {}, byCategory = {}, bySubCategory = {};
  let totalExpense = 0, days = new Set();
  ...
  data.forEach((entry) => {
    ...
    bySubCategory[entry.subCategory] = (bySubCategory[entry.subCategory] || 0) + amt;
    totalExpense += amt;
  });

  document.getElementById("dailySpend").innerText = `${currentCurrency} ${(totalExpense / days.size).toFixed(2)}`;
  ...
  new Chart(document.getElementById("pieChart"), {
    type: "pie",
    data: {
      labels: Object.keys(byCategory).map(cat => categoryIcon(cat) + " " + cat),
      datasets: [{
  ...
  sortedExpenses.slice(0, 5).forEach((exp) => {
    ...
    li.textContent = `${categoryIcon(exp.category)} ${exp.category} > ${exp.subCategory}: ${currentCurrency} ${amt}`;
    list.appendChild(li);
  });
}
...
function exportCharts(format) {
  const charts = ["lineChart", "pieChart", "barChart"].map(id => document.getElementById(id));
  if (format === "png") {
